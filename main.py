...
# Join cap per invite link (stored so admins can later editChatInviteLink).
INVITE_MEMBER_LIMIT = 50


def _invite_name(user: dict, tg_id: int) -> str:
    """A safe Telegram invite-link name from the user's display name / username:
    strip odd characters, collapse whitespace, truncate to 32, fall back to id."""
    raw = (user.get("display_name") or user.get("username") or "").strip()
    cleaned = re.sub(r"[^\w \-]", "", raw)
    cleaned = re.sub(r"\s+", " ", cleaned).strip()[:32].strip()
    return cleaned or f"user {tg_id}"


@app.get("/bt/api/invite")
async def invite(user: dict = Depends(require_user)):
    tg_id = user["tg_id"]
    allowed, retry_after = ratelimit.check(
        f"invite:{tg_id}", limit=settings.bt_rl_read_limit, window_sec=settings.bt_rl_read_window_sec)
    if not allowed:
        return _rl_err(retry_after)
    link_row = await db.get_invite_link(tg_id)
    stats = await db.invite_stats(tg_id)
    return {
        "ok": True,
        "link": (link_row or {}).get("link"),
        "member_limit": int((link_row or {}).get("member_limit") or INVITE_MEMBER_LIMIT),
        "referred_count": stats["referred_count"],
        "total_earned": stats["total_earned"],
    }


@app.post("/bt/api/invite/generate")
async def invite_generate(user: dict = Depends(require_user)):
    tg_id = user["tg_id"]
    allowed, retry_after = ratelimit.check(f"invite_gen:{tg_id}", limit=10, window_sec=60)
    if not allowed:
        return _rl_err(retry_after)

    # Fast path: a returning user already has a link — skip the advisory lock.
    existing = await db.get_invite_link(tg_id)
    if not (existing and existing.get("link")):
        # Race-safe create: db holds a per-user advisory lock across the Telegram
        # call so exactly one link is ever created per user (idempotent).
        name = _invite_name(user, tg_id)

        async def _create():
            return await notify.create_chat_invite_link(
                settings.bt_partygc_chat_id, name, INVITE_MEMBER_LIMIT)

        existing = await db.generate_invite_link(
            tg_id, name, INVITE_MEMBER_LIMIT, _create)
    if not (existing and existing.get("link")):
        return {"ok": False, "error": "link_failed"}

    # Return the same shape as GET (link + member_limit + stats) so both layers
    # share one contract.
    stats = await db.invite_stats(tg_id)
    return {
        "ok": True,
        "link": existing["link"],
        "member_limit": int(existing.get("member_limit") or INVITE_MEMBER_LIMIT),
        "referred_count": stats["referred_count"],
        "total_earned": stats["total_earned"],
    }
